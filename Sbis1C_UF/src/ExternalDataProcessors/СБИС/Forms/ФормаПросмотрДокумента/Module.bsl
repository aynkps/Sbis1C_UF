...
&НаСервере
Процедура аян_ДоработатьФорму()
	
	// + Команда  аян_ДополнитьДокумент
	//ДобавитьРазместитьКомандуФормы(Форма, "Склонения", "Подключаемый_Склонения", "ГруппаОсновныеСведения",
	пр_Общий.ДобавитьРазместитьКомандуФормы(ЭтаФорма, "аян_ДополнитьДокумент", "аян_ДополнитьДокумент" ,
												"ТабличнаяЧастьКоманднаяПанель", , "*Дополнить документ 1с", "");
	
	//аян_КоличествоВПересчете
	аян_СозданиеНаФормеРеквизитаТЧ("ТабличнаяЧасть", "аян_КоличествоВПересчете", "*КолВПересчете", ОбщегоНазначения.ОписаниеТипаЧисло(15,3), "ТабличнаяЧасть",
										"ТабличнаяЧастьКоличество");
	
	//аян_КоличествоБыло
	аян_СозданиеНаФормеРеквизитаТЧ("ТабличнаяЧасть", "аян_КоличествоБыло", "*КолВДок", ОбщегоНазначения.ОписаниеТипаЧисло(15,3), "ТабличнаяЧасть",
										"аян_КоличествоВПересчете");
	
	
	//аян_ЦенаБыло
	аян_СозданиеНаФормеРеквизитаТЧ("ТабличнаяЧасть", "аян_ЦенаБыло", "*ЦенаБыло", ОбщегоНазначения.ОписаниеТипаЧисло(15,3), "ТабличнаяЧасть",
										"ТабличнаяЧастьСуммаБезНал");
	
	// + Реквизит "аян_Добавлен" в ТЧ.ТабличнаяЧасть
	аян_СозданиеНаФормеРеквизитаТЧ("ТабличнаяЧасть", "аян_Добавлен", "*Добавлен", Новый ОписаниеТипов("Булево"), "ТабличнаяЧасть",
										"аян_КоличествоБыло", ВидПоляФормы.ПолеФлажка, "аян_ДобавленПриИзменении");
	
	Элементы.Переместить(Элементы["ТабличнаяЧастьСумма"],  Элементы["ТабличнаяЧасть"], Элементы["ТабличнаяЧастьСуммаБезНал"]);	
	
	Элементы.ТабличнаяЧастьХарактеристикаПоставщика.Видимость = Ложь;
	Элементы.ТабличнаяЧастьХарактеристика.Видимость = Ложь;
	
КонецПроцедуры
 
&НаСервере
Процедура аян_СозданиеНаФормеРеквизитаТЧ(ИмяТЧ, ИмяПоля, Заголовок, Тип, ИмяГруппы, ПоместитьПеред = "", ВидПоляФормы = Неопределено, Действие = "")
	
	//Заголовок = СтрЗаменить(ИмяПоля, "аян_", "*");
	пр_Общий.ДобавитьРеквизитФормы(ЭтаФорма, ИмяПоля, Тип, Заголовок, ИмяТЧ,, ИмяТЧ, ИмяТЧ + ИмяПоля);
	
	аян_СтруктураРеквизита = Новый Структура(); 
	аян_СтруктураРеквизита.Вставить("ПутьКДанным", СтрШаблон("%1.%2", ИмяТЧ, ИмяПоля)); //"ТабличнаяЧасть.аян_Добавлен");
	аян_СтруктураРеквизита.Вставить("ИмяГруппы", ИмяГруппы);//"ТабличнаяЧасть");
	
	Если ЗначениеЗаполнено(ПоместитьПеред)  Тогда
		аян_СтруктураРеквизита.Вставить("ПоместитьПеред", ПоместитьПеред);//"ТабличнаяЧастьХарактеристика");
	КонецЕсли;
	
	Если ВидПоляФормы <> Неопределено Тогда
		аян_СтруктураРеквизита.Вставить("ВидПоляФормы", ВидПоляФормы);//ВидПоляФормы.ПолеФлажка);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Действие)  Тогда
		аян_Событие = Новый Массив;
		аян_Событие.Добавить(Новый Структура("Событие, Действие", "ПриИзменении", Действие));//"аян_ДобавленПриИзменении")); 
		аян_СтруктураРеквизита.Вставить("ОбработкаСобытий", аян_Событие);
	КонецЕсли;
	
	аян_МассивРеквизитов = Новый Массив;
	аян_МассивРеквизитов.Добавить(Новый Структура("Имя, Значение", ИмяПоля, аян_СтруктураРеквизита));
	пр_Общий.РазместитьРеквизитыНаФорме(ЭтаФорма, аян_МассивРеквизитов);
	
КонецПроцедуры

//TODO:  
&НаКлиенте
Процедура аян_ДобавленПриИзменении()
	
	аян_ТекЭлемент = Элементы.ТабличнаяЧасть.ТекущиеДанные;
	
	Если аян_ТекЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура аян_ДополнитьДокумент(Команда)
	
	//Если сбисЭлементФормы(ЭтаФорма,"ПодготовитьКЗагрузке").Видимость = Истина Тогда
	//	Сообщение = Новый СообщениеПользователю;
	//	Сообщение.Текст = "Документ не подготовлен к загрузке";
	//	Сообщение.Сообщить(); 
	//	Возврат;
	//КонецЕсли;
	
	аян_МассивСтрок = ТабличнаяЧасть.НайтиСтроки(Новый Структура("Отмечен", Истина));
	Если аян_МассивСтрок.Количество() = 0  Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не выбранны строки для добавления";
		Сообщение.Сообщить(); 
		Возврат;
	КонецЕсли; 
	
	аян_ДополнитьДокументНаСервере();
	
КонецПроцедуры

&НаСервере
Функция аян_ТекстЗапросаДополненияДокумента()
	Возврат
		"ВЫБРАТЬ
		|	Данные.Название КАК Название,
		|	ВЫРАЗИТЬ(Данные.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
		|	Данные.ЕдИзм КАК ЕдИзм,
		|	Данные.Количество КАК Количество,
		|	Данные.Цена КАК Цена,
		|	Данные.Сумма КАК Сумма,
		|	ВЫРАЗИТЬ(Данные.СтавкаНДС КАК СТРОКА(10)) КАК СтавкаНДС,
		|	Данные.СуммаНДС КАК СуммаНДС,
		|	Данные.Идентификатор КАК Идентификатор,
		|	Данные.Отмечен КАК Отмечен,
		|	Данные.аян_Добавлен КАК аян_Добавлен,
		|	Данные.НомерПП КАК НомерПП
		|ПОМЕСТИТЬ ВТ_ДобавляемаяНоменклатура
		|ИЗ
		|	&Данные КАК Данные
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ДобавляемаяНоменклатура.Номенклатура КАК Номенклатура,
		|	ВТ_ДобавляемаяНоменклатура.Количество КАК Количество,
		|	ВТ_ДобавляемаяНоменклатура.Количество КАК КоличествоУпаковок,
		|	ВТ_ДобавляемаяНоменклатура.Цена КАК Цена,
		|	ВТ_ДобавляемаяНоменклатура.СуммаНДС КАК Сумма,
		|	СтавкиНДС.Ссылка КАК СтавкаНДС,
		|	ВТ_ДобавляемаяНоменклатура.СуммаНДС КАК СуммаНДС,
		|	ВТ_ДобавляемаяНоменклатура.Сумма КАК СуммаСНДС,
		|	ВТ_ДобавляемаяНоменклатура.ЕдИзм КАК ЕдИзм,
		|	ВТ_ДобавляемаяНоменклатура.Идентификатор КАК Идентификатор,
		|	ВТ_ДобавляемаяНоменклатура.Отмечен КАК Отмечен,
		|	ВТ_ДобавляемаяНоменклатура.аян_Добавлен КАК аян_Добавлен,
		|	ВТ_ДобавляемаяНоменклатура.НомерПП КАК НомерПП,
		|	ЕСТЬNULL(ПриобретениеТоваровУслугТовары.НомерСтроки, НЕОПРЕДЕЛЕНО) КАК НомерСтроки,
		|	ВТ_ДобавляемаяНоменклатура.СтавкаНДС КАК СтавкаНДС_Н,
		|	ВТ_ДобавляемаяНоменклатура.Название КАК Название,
		|	ПриобретениеТоваровУслуг.Склад КАК Склад
		|ИЗ
		|	ВТ_ДобавляемаяНоменклатура КАК ВТ_ДобавляемаяНоменклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг.Товары КАК ПриобретениеТоваровУслугТовары
		|		ПО ВТ_ДобавляемаяНоменклатура.Номенклатура = ПриобретениеТоваровУслугТовары.Номенклатура
		|			И (ПриобретениеТоваровУслугТовары.Ссылка = &ДокСсылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтавкиНДС КАК СтавкиНДС
		|		ПО ВТ_ДобавляемаяНоменклатура.СтавкаНДС = СтавкиНДС.Наименование
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ПриобретениеТоваровУслуг
		|		ПО (ПриобретениеТоваровУслуг.Ссылка = &ДокСсылка)
		|ГДЕ
		|	ВТ_ДобавляемаяНоменклатура.Отмечен = ИСТИНА
		|	И ВТ_ДобавляемаяНоменклатура.аян_Добавлен = ЛОЖЬ
		|";
	
КонецФункции 

&НаСервере
Функция аян_ДополнитьДокументНаСервере()	Экспорт
	
	МассивДок = Новый Массив;
	Для каждого аян_СтрТабДок Из ТаблицаДокументов Цикл
		Для каждого аян_Док Из аян_СтрТабДок.Документы1С Цикл
			аян_ДокЗнач = аян_Док.Значение;
			Если ТипЗнч(аян_ДокЗнач) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
				МассивДок.Добавить(аян_ДокЗнач);
			КонецЕсли;
		КонецЦикла; 	
	КонецЦикла; 	
	
	КолДок = МассивДок.Количество(); 
	Если КолДок = 0  Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Нет документа приобретение";
		Сообщение.Сообщить(); 
		Возврат Неопределено;
	ИначеЕсли КолДок > 1 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон("Нельзя добавить (распределить) номенклатуру. Так как количество документов = %1 ", КолДок);
		Сообщение.Сообщить(); 
		Возврат Неопределено;
	КонецЕсли; 

	аян_ДокЗнач = МассивДок[0];
	Запрос = Новый Запрос;
	Запрос.Текст = аян_ТекстЗапросаДополненияДокумента(); 
	Запрос.УстановитьПараметр("Данные", ТабличнаяЧасть.Выгрузить());
	Запрос.УстановитьПараметр("ДокСсылка", аян_ДокЗнач);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВДЗ = РезультатЗапроса.Выбрать();
	
	//TODO:  для одного дока
	аян_ДокОбъект = аян_ДокЗнач.ПолучитьОбъект();
	
	//Если НЕ ЗначениеЗаполнено(аян_ДокОбъект.Склад)  Тогда
	//	аян_ДокОбъект.Склад = аян_ОбщийСерверПовтИсп.СкладТомскАТ_Общий(); 	
	//	
	//	СтруктураОтветственного = ЗакупкиСервер.ПолучитьОтветственногоПоСкладу(аян_ДокОбъект.Склад, аян_ДокОбъект.Менеджер);
	//	Если СтруктураОтветственного <> Неопределено Тогда
	//		аян_ДокОбъект.Принял = СтруктураОтветственного.Ответственный;
	//		аян_ДокОбъект.ПринялДолжность = СтруктураОтветственного.ОтветственныйДолжность;
	//	КонецЕсли;
	//
	//	СкладыСервер.ЗаполнитьСкладыВТабличнойЧасти(аян_ДокОбъект.Склад, Ложь, аян_ДокОбъект.Товары, Истина);
	//
	//КонецЕсли; 
	
	
	//аян_ДокОбъект.ЦенаВключаетНДС = Истина;
	аян_Товары = аян_ДокОбъект.Товары;
	
	
	СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(аян_ДокОбъект);
	...
	//Не корректно заполняет Ставку НДС
	//СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект1СЗначение));
	...
	//								аян_ДокОбъект.ПоступлениеПоЗаказам, аян_ДокОбъект.ТребуетсяЗалогЗаТару));
	
	//СкладГруппа = Справочники.Склады.ЭтоГруппаИСкладыИспользуютсяВТЧДокументовЗакупки(аян_ДокОбъект.Склад);
	//СтруктураДействий.Вставить("ПроверитьЗаполнитьСклад", ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(аян_ДокОбъект, СкладГруппа));
	
	//КэшированныеЗначения = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	
	аян_МассивСтрокДляИзменения = Новый Массив;
							 
	Пока ВДЗ.Следующий() Цикл
		
		Если ВДЗ.НомерСтроки <> Неопределено Тогда
		    Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон("Из строки %1 уже есть заполнение в документе. Номер строки документа %2", ВДЗ.НомерПП, ВДЗ.НомерСтроки);
			Сообщение.Сообщить(); 
			
			аян_МСтрок = ТабличнаяЧасть.НайтиСтроки(Новый Структура("НомерПП", ВДЗ.НомерПП));
			аян_ИзменитьСтрокиТабличнойЧасти(аян_МСтрок);
			
			Продолжить;
		КонецЕсли;  
			
		аян_Нстр = аян_Товары.Добавить();
		ЗаполнитьЗначенияСвойств(аян_Нстр, ВДЗ);
		ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(аян_Нстр, СтруктураДействий, Неопределено);
		
		аян_МСтрок = ТабличнаяЧасть.НайтиСтроки(Новый Структура("НомерПП", ВДЗ.НомерПП));
		Если аян_МСтрок.Количество() > 0  Тогда
			Для каждого аян_Стр Из аян_МСтрок Цикл
				аян_МассивСтрокДляИзменения.Добавить(аян_Стр);
			КонецЦикла; 
		КонецЕсли; 
		
	КонецЦикла;
	
	Если аян_МассивСтрокДляИзменения.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Попытка
		аян_ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
	Исключение
		Аян_ОписОшибик = ОписаниеОшибки();
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = Аян_ОписОшибик;
		Сообщение.Сообщить();
		Возврат Неопределено;
	КонецПопытки;
	
	аян_ИзменитьСтрокиТабличнойЧасти(аян_МассивСтрокДляИзменения);
	
КонецФункции 

&НаСервере
Процедура аян_ИзменитьСтрокиТабличнойЧасти(аян_МассивСтрокДляИзменения)
	
	Если аян_МассивСтрокДляИзменения.Количество() > 0 Тогда
		Для каждого аян_Стр Из аян_МассивСтрокДляИзменения Цикл
			аян_Стр.Отмечен = Ложь;
			аян_Стр.аян_Добавлен = Истина;
		КонецЦикла; 
	КонецЕсли; 
	...
	Если Найти(сбисЭлементФормы(ЭтаФорма,"ТаблицаДокументов").ТекущиеДанные.Статус, "Сопоставление не требуется")=0 Тогда
		...
		Если ЗначениеЗаполнено(ТаблДок) Тогда 
			...
			Для Каждого СтрТабл Из ТаблДок Цикл
				...
				// ▼ ayan#bitrix2889 Добавление строк в документ 1С из обработки СБИС  КирилловПС  2023.07.12 ▼ Начало
				//Если  Тогда
					СтрТабл.Свойство("аян_КоличествоБыло", НоваяСтрока.аян_КоличествоБыло);
					СтрТабл.Свойство("аян_КоличествоВПересчете", НоваяСтрока.аян_КоличествоВПересчете);
					СтрТабл.Свойство("аян_ЦенаБыло", НоваяСтрока.аян_ЦенаБыло);
				//КонецЕсли; 
				// ▲ ayan#bitrix2889 Добавление строк в документ 1С из обработки СБИС  КирилловПС  2023.07.12 ▲ Конец 
