...
Функция МассивСтруктурВТаблицуЗначений(МассивСтруктур)
	...
	// ▼ ayan# Устранение ошибки СБИС ▼ Начало
	//Если МассивСтруктур = Неопределено Или МассивСтруктур.Количество() = 0 Тогда
	Если МассивСтруктур = Неопределено Тогда
		Возврат Результат;
	ИначеЕсли МассивСтруктур.Количество() = 0 Тогда
		Возврат Результат;
	// ▲ ayan# Устранение ошибки СБИС ▲ Конец 
	Иначе 
...
Процедура ФильтрОбновитьПанель() Экспорт
	...
	// ▼ ayan#bitrix1075_03#УТ2021 Добавление доп отборов з П.С.  2022.08.25 ▼ Начало
	Если ЗначениеЗаполнено(аян_ФильтрКонтрагентНастроенЭДОСБИС) Тогда
		ТекущееЗначениеФильтра = ТекущееЗначениеФильтра + ", КонтрагентНастроенЭДОСБИС:" + строка(аян_ФильтрКонтрагентНастроенЭДОСБИС);	
	КонецЕсли;
	
	Если ЗначениеЗаполнено(аян_ФильтрСписокВидовНоменклатур) Тогда 
		аян_СписокВидНом = "ВидыНоменклатур:";
		Для каждого ВидНом Из аян_ФильтрСписокВидовНоменклатур Цикл
			аян_СписокВидНом = аян_СписокВидНом + ВидНом.Представление + ",";
		КонецЦикла;
		ТекущееЗначениеФильтра = ТекущееЗначениеФильтра + ", " + Лев(аян_СписокВидНом,СтрДлина (аян_СписокВидНом)-1);	
	конецесли;
	// ▲ ayan#bitrix1075_03#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▲ Конец 
...
Функция ТаблДокПолучитьВыбранныеСтроки() Экспорт
	...
	Если СписокСтрок.Количество()=0 и Кэш.ТаблДок.ТекущаяСтрока<>Неопределено Тогда
		// ▼ ayan#bitrix1177_04#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▼ Начало
		Если Кэш.Текущий.ТипДок = "УПД" И Кэш.ТаблДок.ТекущиеДанные.аян_ЗаполненШК = Ложь И аян_ОтправлятьТолькоЗаполненныеШК Тогда
			Возврат СписокСтрок;
		КонецЕсли; 
		// ▲ ayan#bitrix1177_04#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▲ Конец 
...
Процедура ОтправитьВыбранныеДокументы(Ответ, СбисДополнительныеПараметры=Неопределено) Экспорт
	...
	// ▼ ayan#bitrix1253#УТ2021_01 Регистрация в УТАП признака в Документах Реализации "Отправлен в СБИС" Кириллов П.С.  2022.09.22 ▼ Начало
	аян_ЗарегистрироватьСтатусыДокументовВУТАП(Кэш.Текущий.ТипДок, СписокОтмеченныхДокументов);
	Если РежимОтладки ИЛИ СписокОтмеченныхДокументов.Количество() = 0 Тогда
		Возврат;	
	КонецЕсли; 
	// ▲ ayan#bitrix1253#УТ2021_01 Регистрация в УТАП признака в Документах Реализации "Отправлен в СБИС" Кириллов П.С.  2022.09.22 ▲ Конец 
...
Функция сбисСохранитьТекущийФильтр(Кэш) Экспорт
	...
	// ▼ ayan#bitrix1075_04#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▼ Начало
	//Если Кэш.Текущий.ТипДок = "РеализацияТоваровУслуг" Тогда
	аян_типДок = Кэш.Текущий.ТипДок;
	Если  (аян_типДок = "РеализацияТоваровУслуг" ИЛИ 
							  аян_типДок = "УПД" ИЛИ 
							  аян_типДок = "EDI_УКД" ИЛИ 
							  аян_типДок = "EDI_УПД") Тогда
		
		ТекущийФильтр.Вставить("аян_ФильтрСписокВидовНоменклатур", аян_ФильтрСписокВидовНоменклатур);
		ТекущийФильтр.Вставить("аян_ФильтрКонтрагентНастроенЭДОСБИС", аян_ФильтрКонтрагентНастроенЭДОСБИС);
	//Иначе
	//	аян_ФильтрСписокВидовНоменклатур = Ложь;
	//	аян_ФильтрКонтрагентНастроенЭДОСБИС = Новый СписокЗначений();
	КонецЕсли; 
	// ▲ ayan#bitrix1075_04#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▲ Конец 
...
// ▼ ayan#bitrix1253#УТ2021_02 Регистрация в УТАП признака в Документах Реализации "Отправлен в СБИС" Кириллов П.С.  2022.09.22 ▼ Начало
&НаКлиенте
Процедура аян_ЗарегистрироватьСтатусыДокументовВУТАП(ТипДок, СписокОтмеченныхДокументов)
	
	Если НЕ (ТипДок = "УПД" ИЛИ ТипДок = "РеализацияТоваровУслуг") Тогда
		Возврат;	
	КонецЕсли; 
	
	аян_МассивДанных = Новый Массив;
	
	//аян_СписокОтмеченныхДокументов = Новый СписокЗначений;
	Для каждого ЭлСписка Из СписокОтмеченныхДокументов Цикл
		
		ЗначЭлСписка = ЭлСписка.Значение;
		
		// ▼ ayan#bitrix1177_03#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▼ Начало
		Если ТипДок <> "УПД" И ЗначЭлСписка.аян_ЗаполненШК = Ложь И аян_ОтправлятьТолькоЗаполненныеШК Тогда
			СписокОтмеченныхДокументов.Удалить(ЗначЭлСписка);	
		КонецЕсли; 
		// ▲ ayan#bitrix1177_03#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▲ Конец 
		
		аян_МассивДанных.Добавить(ЗначЭлСписка.Документы1С[0].Значение);
		
	КонецЦикла; 
	
	аян_РезультатОтправкиВУТАП = аян_ЗарегистрироватьСтатусыДокументовВУТАП_Сервер(аян_МассивДанных, РежимОтладки);

	Если аян_РезультатОтправкиВУТАП = "Статус изменен во всех объектах"  Тогда
		Возврат;
	ИначеЕсли  аян_РезультатОтправкиВУТАП = "Нет данных" Тогда
		СписокОтмеченныхДокументов = Новый СписокЗначений;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не удалось установить связь с УТАП";
		Сообщение.Сообщить();  
		Возврат;
	КонецЕсли;
	
	Если аян_РезультатОтправкиВУТАП = Неопределено Тогда
		СписокОтмеченныхДокументов = Новый СписокЗначений;
		Возврат;
	КонецЕсли;
	
	МассивКУдалению = Новый Массив;
	Для каждого	Стр  Из СписокОтмеченныхДокументов Цикл
		
		ЗначениеЭлем = Стр.Значение.Документы1С[0].Значение;
		Для каждого НеЗарегистрированныйДок Из аян_РезультатОтправкиВУТАП Цикл
			Если ЗначениеЭлем = НеЗарегистрированныйДок Тогда
	...
	Для каждого Стр Из МассивКУдалению Цикл
		СписокОтмеченныхДокументов.Удалить(Стр);
	КонецЦикла; 
	
КонецПроцедуры

&НаСервере
Функция аян_ЗарегистрироватьСтатусыДокументовВУТАП_Сервер(аян_МассивДанных, РежимОтладки)

	Если РежимОтладки Тогда
		ПолныйПуть = "\\pivo.local\Resource\Личные\Кириллов ПС\r\ut_2021\work\аян_ОбменДанными.epf";
		Обработка = аян_Разработка.ПолучитьВнешнююОбработкуПоПути(ПолныйПуть);
		аян_РезультатОтправкиВУТАП = Обработка.ЗарегистрироватьСтатусСБИСВУТАП(аян_МассивДанных, "Отправлен", РежимОтладки);
	Иначе 	
		аян_РезультатОтправкиВУТАП = аян_ОбменДанными.ЗарегистрироватьСтатусСБИСВУТАП(аян_МассивДанных, "Отправлен", РежимОтладки);
		//аян_РезультатОтправкиВУТАП = ЗарегистрироватьСтатусСБИСВУТАП(аян_МассивДанных, "Отправлен", РежимОтладки);
	КонецЕсли; 
	
	Возврат  аян_РезультатОтправкиВУТАП;
	
КонецФункции

// ▲ ayan#bitrix1253#УТ2021_02 Регистрация в УТАП признака в Документах Реализации "Отправлен в СБИС" Кириллов П.С.  2022.09.22 ▲ Конец 
