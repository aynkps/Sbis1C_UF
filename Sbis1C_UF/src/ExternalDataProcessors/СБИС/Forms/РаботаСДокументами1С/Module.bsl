...
Функция ЗаполнитьАтрибуты(Кэш,Раздел,Ини,СтруктураДокумента) Экспорт
	...
	Для Каждого Параметр Из ини Цикл
		Если  Лев(Параметр.Ключ,Длина)=Раздел Тогда
			...
			// ▼ ayan#bitrix1303 Разделение ВСД по способу транспортировки. Кириллов П.С.  2022.11.22 ▼ Начало
			//Если Параметр.Ключ = "ДокументОснование_Номер" И ЗначениеЗаполнено(ЗначениеПараметра) Тогда
			//	аян_Номер = Ини.ДокОснование.ДокументОснование.Номер;
			//	Если СтрНайти(аян_Номер, ЗначениеПараметра) Тогда
			//		ЗначениеПараметра = аян_Номер;
			//	КонецЕсли; 
			//	ЗначениеПараметра = Ини.ДокОснование.ДокументОснование.Номер 
			//КонецЕсли; 	
			 //▲ ayan#bitrix1303 Разделение ВСД по способу транспортировки. Кириллов П.С.  2022.11.22 ▲ Конец 
...
Функция ПолучитьФильтр(Кэш)
	...
	// ▼ ayan#bitrix1075_05#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▼ Начало
	ПараметрыФильтра.Вставить("аян_ФильтрСписокВидовНоменклатур", ГлавноеОкно.аян_ФильтрСписокВидовНоменклатур); 
	ПараметрыФильтра.Вставить("аян_ФильтрКонтрагентНастроенЭДОСБИС", ГлавноеОкно.аян_ФильтрКонтрагентНастроенЭДОСБИС); 
	// ▲ ayan#bitrix1075_05#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▲ Конец 
...
Функция сбисОбновитьРеестрДокументов1С(Ини, Кэш) Экспорт
	...
	Если фрм = Ложь Тогда
		...
		// ▼ ayan#bitrix1075_06#УТ2021 Добавление доп отборов Кириллов П.С.  2023.01.20 ▼ Начало
		//МассивДокументов1С = ЗаполнитьМассивДокументов1С(ТекстЗапроса, ПараметрыФильтра, Кэш.Парам.КаталогНастроек);
		МассивДокументов1С = ЗаполнитьМассивДокументов1С(ТекстЗапроса, ПараметрыФильтра, Кэш.Парам.КаталогНастроек, Кэш.Текущий.ТипДок);
		// ▲ ayan#bitrix1075_06#УТ2021 Добавление доп отборов Кириллов П.С.  2023.01.20 ▲ Конец 
	Иначе
...
Процедура НайтиПодходящиеДокументыОпределенногоТипа(СписокДокументов, Ини, КэшИни, Парам, оДокумент) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаДатаВх) Тогда
		Если Парам.СопоставлениеПериод = "Дата документа" Тогда
			// ▼ ayan#bitrix#4397 Создавать документ Поступление в тек дате КирилловПС  2023.12.08  ▼ Начало
			//Дату документу ставит текущюю, ищем только по входящей
			Если НЕ (Ини.Документ.Значение = "Документ.ПриобретениеТоваровУслуг" ИЛИ Ини.Документ.Значение = "Документ.СчетФактураПолученный") Тогда
			...
			КонецЕсли; 	
			// ▲ ayan#bitrix#4397 Создавать документ Поступление в тек дате КирилловПС  2023.12.08  ▲ Конец 
...
Функция ЗаполнитьДанныеДляЗагрузкиПоУмолчанию(Кэш, СоставПакета, Вложение, ДополнительныеПараметры=Неопределено) Экспорт     	
	...
	Если ПараметрыНоменклатуры.НадоСопоставлять 
		...
		Для Каждого СтрокаНоменклатуры Из ТаблДок Цикл
			...
			ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, СтрокаНоменклатуры, , "ИдентификаторДокумента");
		...
		Если ЗначениеЗаполнено(ТаблДок) Тогда
			...
			Для Каждого СтрТабл Из ТаблДок Цикл
				...
				Если ЗначениеЗаполнено(ДанныеНоменклатуры) Тогда
					...
					//Не оптимальный костыль
					Если Вложение.ФорматДляЗагрузки = "УпдСчфДоп"  Тогда
						аян_ПересчитатьКоличество(ТаблДок[счСтр]);
					КонецЕсли; 
...
Функция СоздатьПерезаполнитьДокументы(Кэш,СоставПакета,СписокВложений,Параметры,ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаТаблДок из СписокВложений Цикл
		...
		ИначеЕсли МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры") 
			И Вложение.Свойство("ДанныеРасширенногоСопоставления") Тогда // TO DO Ду-ду (с)Сыч
			
			ДанныеРасширенногоСопоставления = Вложение.ДанныеРасширенногоСопоставления;
			ДопПараметры = Новый Структура("ТаблДок", ТаблДок);
			ТаблДок = МодульОбъектаКлиент().СтрокаСопоставленияСБИССервер_СформироватьТаблДок(ДанныеРасширенногоСопоставления, ДопПараметры);
...
&НаСервереБезКонтекста
// ▼ ayan#bitrix1075_05.1#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▼ Начало
//Функция ЗаполнитьМассивДокументов1С(ТекстЗапроса, ПараметрыФильтра, КаталогНастроек=Неопределено) Экспорт
Функция ЗаполнитьМассивДокументов1С(ТекстЗапроса, ПараметрыФильтра, КаталогНастроек = Неопределено, ТипДок = "") Экспорт
	аян_ЕстьИзмененияВТекстеЗапроса = Ложь;
// ▲ ayan#bitrix1075_05.1#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▲ Конец 
	Запрос = Новый Запрос;
	Если НЕ ПараметрыФильтра.ВсеДокументы и ПараметрыФильтра.РазмерСтраницы<>"все" и ЗначениеЗаполнено(ПараметрыФильтра.РазмерСтраницы) и ПараметрыФильтра.Страница<>0 и Найти(нрег(ТекстЗапроса),"выбрать разрешенные") = 1 и ПараметрыФильтра.ФильтрСостояние = "Все документы" Тогда 
		...
		// ▼ ayan#bitrix1075_06#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▼ Начало
		аян_ЕстьИзмененияВТекстеЗапроса = аян_ЕстьИзмененияВТекстеЗапроса(ПараметрыФильтра, ТипДок, Запрос, ТекстЗапроса);
		// ▲ ayan#bitrix1075_06#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▲ Конец 
	...
	// ▼ ayan#bitrix1075_06#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▼ Начало
	Если аян_ЕстьИзмененияВТекстеЗапроса  Тогда
		ТекстЗапроса = аян_ТекстЗапросаШК() + Символы.ПС + ТекстЗапроса;
	ИначеЕсли аян_ЕстьИзмененияВТекстеЗапроса(ПараметрыФильтра, ТипДок, Запрос, ТекстЗапроса) Тогда 
		ТекстЗапроса = аян_ТекстЗапросаШК() + Символы.ПС + ТекстЗапроса;
	КонецЕсли; 
	// ▲ ayan#bitrix1075_06#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▲ Конец 
...
#Область  аян_Доработки


// ▼ ayan#bitrix1075_07#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▼ Начало
&НаСервереБезКонтекста
Процедура аян_ПодготовитьЗапрос(ТекстЗапроса, аян_СтрокаПоиска, аян_СтрокаВставки, аян_ВставитьДо = Истина, аян_ПС = Истина)
	
	Если аян_ПС = Истина Тогда
		аян_ПС = Символы.ПС;
	Иначе	
		аян_ПС = " ";
	КонецЕсли; 
	
	аян_ПоискПоз = Найти(ТекстЗапроса, аян_СтрокаПоиска);
	аян_ТекстДо = Лев(ТекстЗапроса, аян_ПоискПоз - 1);
	
	Если аян_ВставитьДо = Истина Тогда
		аян_ТекстПосле = Сред(ТекстЗапроса, аян_ПоискПоз);
		ТекстЗапроса = аян_ТекстДо + аян_СтрокаВставки + аян_ПС + аян_ТекстПосле;
	Иначе 	
		аян_ДлинаВставки = СтрДлина(аян_СтрокаПоиска);
		аян_ТекстПосле = Сред(ТекстЗапроса, аян_ПоискПоз + аян_ДлинаВставки);
		ТекстЗапроса = аян_ТекстДо + " " + аян_СтрокаПоиска + аян_ПС + аян_СтрокаВставки + аян_ТекстПосле;
	КонецЕсли; 
	
КонецПроцедуры
// ▲ ayan#bitrix1075_07#УТ2021 Добавление доп отборов Кириллов П.С.  2022.08.25 ▲ Конец 

// ▼ ayan#bitrix1177_02#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▼ Начало
&НаСервереБезКонтекста
Функция аян_ТекстЗапросаШК()
	Возврат
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	Товары.Ссылка КАК Документ,
		|	МАКСИМУМ(СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияПодконтрольнаяВЕТИС)) КАК МолочнаяПродукцияВЕТИС,
		|	МАКСИМУМ(СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияБезВЕТИС)) КАК МолочнаяПродукцияБезВЕТИС,
		...
		|ГДЕ
		|	СправочникНоменклатура.ОсобенностьУчета В (ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияПодконтрольнаяВЕТИС))
		|	И Товары.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон
		|	И (Товары.Ссылка.Организация = &ТекущаяОрганизация
		|			ИЛИ НЕ &ЗначениеТекущаяОрганизацияЗаполнено)
		...
		|	РеализацияТоваровУслугШтрихкодыУпаковок.Ссылка КАК СсылкаШК,
		|	РеализацияТоваровУслугСерии.Ссылка КАК СсылкаСерии,
		|	ВТ_Товары.МолочнаяПродукцияВЕТИС КАК МолочнаяПродукцияВЕТИС,
		|	ВТ_Товары.МолочнаяПродукцияБезВЕТИС КАК МолочнаяПродукцияБезВЕТИС,
		|	РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
		|	NULL КАК ШтрихкодУпаковкиБезВЕТИС,
		|	ВТ_Товары.Документ КАК СсылкаШКБезВЕТИС
		|ПОМЕСТИТЬ ВТ_СвязьСШКНоменклатура
		|ИЗ
		|	ВТ_Товары КАК ВТ_Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.Серии КАК РеализацияТоваровУслугСерии
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.ШтрихкодыУпаковок КАК РеализацияТоваровУслугШтрихкодыУпаковок
		|			ПО РеализацияТоваровУслугСерии.Номенклатура = РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки.Номенклатура
		|				И РеализацияТоваровУслугСерии.Ссылка = РеализацияТоваровУслугШтрихкодыУпаковок.Ссылка
		|				И РеализацияТоваровУслугСерии.Характеристика = РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки.Характеристика
		|				И РеализацияТоваровУслугСерии.Серия = РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки.Серия
		|				И РеализацияТоваровУслугСерии.Количество = РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки.Количество
		|		ПО ВТ_Товары.Документ = РеализацияТоваровУслугСерии.Ссылка
		|			И ВТ_Товары.Номенклатура = РеализацияТоваровУслугСерии.Номенклатура
		|			И ВТ_Товары.Характеристика = РеализацияТоваровУслугСерии.Характеристика
		|ГДЕ
		|	ВТ_Товары.МолочнаяПродукцияВЕТИС
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ВТ_Товары.Документ,
		|	ВТ_Товары.МолочнаяПродукцияВЕТИС,
		|	ВТ_Товары.МолочнаяПродукцияБезВЕТИС,
		|	NULL,
		|	РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки,
		|	РеализацияТоваровУслугШтрихкодыУпаковок.Ссылка
		|ИЗ
		|	ВТ_Товары КАК ВТ_Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг.ШтрихкодыУпаковок КАК РеализацияТоваровУслугШтрихкодыУпаковок
		|		ПО ВТ_Товары.Документ = РеализацияТоваровУслугШтрихкодыУпаковок.Ссылка
		|			И ВТ_Товары.Номенклатура = РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки.Номенклатура
		|			И ВТ_Товары.Характеристика = РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки.Характеристика
		|			И ВТ_Товары.КоличествоУпаковок = РеализацияТоваровУслугШтрихкодыУпаковок.ШтрихкодУпаковки.Количество
		|ГДЕ
		|	ВТ_Товары.МолочнаяПродукцияБезВЕТИС
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_СвязьСШКНоменклатура.Документ КАК Документ,
		|	ВТ_СвязьСШКНоменклатура.Номенклатура КАК Номенклатура,
		|	ВТ_СвязьСШКНоменклатура.СсылкаШК КАК СсылкаШК,
		|	ВТ_СвязьСШКНоменклатура.СсылкаСерии КАК СсылкаСерии,
		|	ВТ_СвязьСШКНоменклатура.МолочнаяПродукцияВЕТИС КАК МолочнаяПродукцияВЕТИС,
		|	ВТ_СвязьСШКНоменклатура.МолочнаяПродукцияБезВЕТИС КАК МолочнаяПродукцияБезВЕТИС,
		|	ВТ_СвязьСШКНоменклатура.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
		|	ВТ_СвязьСШКНоменклатура.ШтрихкодУпаковкиБезВЕТИС КАК ШтрихкодУпаковкиБезВЕТИС,
		|	ВТ_СвязьСШКНоменклатура.СсылкаШКБезВЕТИС КАК СсылкаШКБезВЕТИС,
		|	НЕ 1 В
		|				(ВЫБРАТЬ ПЕРВЫЕ 1
		|					1
		|				ИЗ
		...
		|				ГДЕ
		|					(ВТ_СвязьСШКНоменклатура.Номенклатура = ОписаниеИС.Номенклатура
		|						ИЛИ ВТ_СвязьСШКНоменклатура.Номенклатура.НаборУпаковок = ОписаниеИС.Номенклатура))
		|		И ВТ_СвязьСШКНоменклатура.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины В (ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Вес), ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Длина), ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Объем), ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Площадь)) КАК ЕстьВес
		|ПОМЕСТИТЬ ВТ_ОпределениеВесовогоТовара
		|ИЗ
		|	ВТ_СвязьСШКНоменклатура КАК ВТ_СвязьСШКНоменклатура
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_СвязьСШКНоменклатура.Документ КАК Документ,
		|	МАКСИМУМ(ВТ_СвязьСШКНоменклатура.СсылкаШК ЕСТЬ NULL) КАК НетШК_ВЕТИС,
		|	МАКСИМУМ(ВТ_СвязьСШКНоменклатура.СсылкаСерии ЕСТЬ NULL) КАК СлетелиСерии,
		|	МАКСИМУМ(ВТ_СвязьСШКНоменклатура.МолочнаяПродукцияВЕТИС) КАК МолочнаяПродукцияВЕТИС,
		|	МАКСИМУМ(ВТ_СвязьСШКНоменклатура.МолочнаяПродукцияБезВЕТИС) КАК МолочнаяПродукцияБезВЕТИС,
		|	МАКСИМУМ(ВТ_СвязьСШКНоменклатура.СсылкаШКБезВЕТИС ЕСТЬ NULL) КАК НетШК_БезВЕТИС,
		|	МАКСИМУМ(ВТ_СвязьСШКНоменклатура.ЕстьВес) КАК ЕстьВес
		|ПОМЕСТИТЬ ВТ_СвязьСШК
		|ИЗ
		|	ВТ_ОпределениеВесовогоТовара КАК ВТ_СвязьСШКНоменклатура
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_СвязьСШКНоменклатура.Документ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_СвязьСШК.Документ КАК ПроверяемыйДокумент,
		|	ВТ_СвязьСШК.Документ.Организация КАК Организация,
		|	ВТ_СвязьСШК.СлетелиСерии КАК СлетелиСерии,
		|	НЕ(ВТ_СвязьСШК.НетШК_ВЕТИС
		|			ИЛИ ВТ_СвязьСШК.НетШК_БезВЕТИС) КАК ЗаполненШК,
		|	ВТ_СвязьСШК.НетШК_ВЕТИС КАК НетШК_ВЕТИС,
		|	ВТ_СвязьСШК.НетШК_БезВЕТИС КАК НетШК_БезВЕТИС,
		|	ВТ_СвязьСШК.МолочнаяПродукцияВЕТИС КАК МолочнаяПродукцияВЕТИС,
		|	ВТ_СвязьСШК.МолочнаяПродукцияБезВЕТИС КАК МолочнаяПродукцияБезВЕТИС,
		|	ЕСТЬNULL(ДополнительныеСведения.Значение, """") КАК ОшибкиЗаполненияШК,
		|	ВТ_СвязьСШК.ЕстьВес КАК ЕстьВес
		|ПОМЕСТИТЬ ВТ_ДанныеОШК
		|ИЗ
		|	ВТ_СвязьСШК КАК ВТ_СвязьСШК
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
		|		ПО ВТ_СвязьСШК.Документ = ДополнительныеСведения.Объект
		|			И (ДополнительныеСведения.Свойство = &Свойство_ОшибкиЗаполненияШК)"
		+
		"
		|;
		|
...
КонецФункции 
// ▲ ayan#bitrix1177_02#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▲ Конец 

&НаСервереБезКонтекста
функция аян_ЕстьИзмененияВТекстеЗапроса(ПараметрыФильтра, ТипДок, Запрос, ТекстЗапроса) 
	
	аян_СтрокаВставки = "";
	Если НЕ ПараметрыФильтра.ВсеДокументы И ПараметрыФильтра.Страница <> 0 И Найти(нрег(ТекстЗапроса),"выбрать разрешенные") = 1 Тогда 
		
		Если  ТипДок = "РеализацияТоваровУслуг" ИЛИ 
			ТипДок = "УПД" ИЛИ 
			//ТипДок = "EDI_УКД" ИЛИ 
			ТипДок = "EDI_УПД"  Тогда
			
			Если ЗначениеЗаполнено(ПараметрыФильтра.аян_ФильтрСписокВидовНоменклатур)  Тогда
				
				аян_ПодготовитьЗапрос(ТекстЗапроса, "РАЗРЕШЕННЫЕ", " РАЗЛИЧНЫЕ ", , Ложь);
				
				аян_СтрокаВставки = "
				|	Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ "; 
				аян_ПодготовитьЗапрос(ТекстЗапроса, "ИЗ", аян_СтрокаВставки, Ложь);
				
				аян_СтрокаВставки = "
				|	ПО РеализацияТоваровУслугТовары.Ссылка = РеализацияТоваровУслуг.Ссылка"; 
				аян_ПодготовитьЗапрос(ТекстЗапроса, "ГДЕ", аян_СтрокаВставки);
				
				аян_СтрокаВставки = "
				|		И РеализацияТоваровУслугТовары.Номенклатура.ВидНоменклатуры В(&аян_ФильтрСписокВидовНоменклатур)"; 
				аян_ПодготовитьЗапрос(ТекстЗапроса, ";", аян_СтрокаВставки);
				
				Запрос.УстановитьПараметр("аян_ФильтрСписокВидовНоменклатур", ПараметрыФильтра.аян_ФильтрСписокВидовНоменклатур);
				
			КонецЕсли;
			
			Если ПараметрыФильтра.аян_ФильтрКонтрагентНастроенЭДОСБИС = Истина  Тогда
				
				аян_СтрокаВставки = "
				|		И РеализацияТоваровУслуг.Контрагент.НастроенЭДОСБИС = &аян_ФильтрКонтрагентНастроенЭДОСБИС"; 
				аян_ПодготовитьЗапрос(ТекстЗапроса, ";", аян_СтрокаВставки);
				
				Запрос.УстановитьПараметр("аян_ФильтрКонтрагентНастроенЭДОСБИС", ПараметрыФильтра.аян_ФильтрКонтрагентНастроенЭДОСБИС);
				
			КонецЕсли;
			
			// ▼ ayan#bitrix1177_01#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▼ Начало
			Если 	ТипДок = "УПД" Или 
				ТипДок = "EDI_УПД"  Тогда 
				
				аян_СтрокаВставки = "
				|	,ЕСТЬNULL(ВТ_ДанныеОШК.ЗаполненШК, ЛОЖЬ) КАК ЗаполненШК,
				|	ЕСТЬNULL(ВТ_ДанныеОШК.ОшибкиЗаполненияШК, ЛОЖЬ) КАК ОшибкиЗаполненияШК"; 
				аян_ПодготовитьЗапрос(ТекстЗапроса, "ПОМЕСТИТЬ ВТ_УПД", аян_СтрокаВставки);
				
				аян_СтрокаВставки = "
				|	,ВТ_УПД.ЗаполненШК КАК аян_ЗаполненШК
				|	,ВТ_УПД.ОшибкиЗаполненияШК КАК аян_ОшибкиЗаполненияШК"; 
				аян_ПодготовитьЗапрос(ТекстЗапроса, "СтатусЭД", аян_СтрокаВставки, Ложь);
				
				аян_СтрокаВставки = "
				|	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеОШК КАК ВТ_ДанныеОШК
				|		ПО ВТ_Реализация.Ссылка = ВТ_ДанныеОШК.ПроверяемыйДокумент"; 
				аян_ПодготовитьЗапрос(ТекстЗапроса, "СвязьДокументов.ДокументОснование", аян_СтрокаВставки, Ложь);
				
				//Иначе 	
				//	
				//	аян_СтрокаВставки = "
				//	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеОШК КАК ВТ_ДанныеОШК
				//	|			ПО (ВТ_ДанныеОШК.ПроверяемыйДокумент = ВТ_УПД.Документ)"; 
				//	аян_ПодготовитьЗапрос(ТекстЗапроса, "ГДЕ", аян_СтрокаВставки);
				//	
				//	аян_СтрокаВставки = "
				//	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеОШК КАК ВТ_ДанныеОШК
				...
				ОсобенностиУчетаНоменклатуры.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияПодконтрольнаяВЕТИС);
				ОсобенностиУчетаНоменклатуры.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияБезВЕТИС);
				ОсобенностиУчетаНоменклатуры.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.УпакованнаяВода);
				
				Запрос.УстановитьПараметр("ОсобенностиУчетаНоменклатуры", ОсобенностиУчетаНоменклатуры);
				
				Запрос.УстановитьПараметр("Свойство_ОшибкиЗаполненияШК", аян_ОбщийСерверПовтИсп.Свойство_ОшибкиЗаполненияШК());
				
			КонецЕсли; 
			
			// ▲ ayan#bitrix1177_01#УТ2021 Признак заполнения GTIN-кодов Кириллов П.С.  2022.09.05 ▲ Конец 
			
		КонецЕсли;
		
	КонецЕсли; 
	
	Возврат аян_СтрокаВставки <> "";
...
//Не оптимальный костыль
&НаКлиенте
Процедура аян_ПересчитатьКоличество(стр_ТабДок)
	
	//стр_ТабДок.ЕдИзм = "шт";
	ЭтоШтучныыйТовар = СтрНайти(стр_ТабДок.ЕдИзм, "шт") > 0 ИЛИ стр_ТабДок.ЕдИзм = "л" ИЛИ стр_ТабДок.ЕдИзм = "кг" ИЛИ стр_ТабДок.ЕдИзм = "пар";
	Если ЭтоШтучныыйТовар Тогда
		Возврат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(стр_ТабДок.Номенклатура) Тогда
		Возврат;
	КонецЕсли;
	Если стр_ТабДок.Свойство("аян_КоличествоВПересчете") Тогда
		Если ЗначениеЗаполнено(стр_ТабДок.аян_КоличествоВПересчете) Тогда //Уже пересчитывали
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	аян_КоличествоВПересчете = КоэффициентЕдиницыДляОтчетов(стр_ТабДок.Номенклатура);
	Если аян_КоличествоВПересчете = Неопределено Тогда
		Возврат;
	КонецЕсли; 	
	
	Если аян_КоличествоВПересчете > 1 Тогда
		стр_ТабДок.Вставить("аян_КоличествоВПересчете", аян_КоличествоВПересчете);	
		стр_ТабДок.Вставить("аян_КоличествоБыло", Число(стр_ТабДок.Кол_во));	
		стр_ТабДок.Вставить("аян_ЦенаБыло", Число(стр_ТабДок.Цена));
		
		//Чтобы не ломать логику
		//стр_ТабДок.Вставить("Коэффициент", аян_КоличествоВПересчете); // не учитывает при создании документа	
		//Доработка
		Кол_во =  стр_ТабДок.аян_КоличествоБыло * аян_КоличествоВПересчете;
		Цена =  Окр(стр_ТабДок.аян_ЦенаБыло / аян_КоличествоВПересчете, 2);
		стр_ТабДок.Вставить("Кол_во", Кол_во);	
		стр_ТабДок.Вставить("Цена", Цена);
...
&НаСервереБезКонтекста
Функция КоэффициентЕдиницыДляОтчетов(СсылкаНоменклатура)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	Если ВДЗ.Следующий() Тогда
		Возврат ВДЗ.КоэффициентЕдиницыДляОтчетов;
	Иначе 
		Возврат Неопределено;
	КонецЕсли; 
